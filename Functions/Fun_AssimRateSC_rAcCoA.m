function [assim_rate,uc1,xc1,exitflag]=Fun_AssimRateSC_rAcCoA(xc_0,pHc,beta,x_c_H2,x_ub,index)
clc;
...
Aineq=zeros(n_rxn1+4,n_met);
bineq=zeros(n_rxn1+4,1);
Aeq=zeros(2,n_met);
beq=zeros(2,1);
lb1=zeros(n_met,1);
ub1=zeros(n_met,1);
...
Aineq(n_rxn1+4,i_atp)=-1;
Aineq(n_rxn1+4,i_adp)=1;
Aineq(n_rxn1+4,i_pi)=1;
Aeq(1,i_pyr)=1;
Aeq(2,i_h2)=1;
...
c=a1*w1_'*S1'-2*a2*ln_Keq1'*W1_*S1'-2*w_r*xc_0';
...
options=optimoptions('fmincon','Algorithm','sqp','EnableFeasibilityMode',true,'SpecifyObjectiveGradient',true,'SubproblemAlgorithm','cg',...
                    'ScaleProblem',true,'StepTolerance',1e-8,'OptimalityTolerance',1e-7,'MaxIterations',10000,'MaxFunctionEvaluations',60000);

[ln_x,obj,exitflag,output]=fmincon(@(ln_x)Fun_Obj(ln_x,S,Keq1,kapp1,varth,rxn,[jc;jf],n_rxn1,n_met),sol.x,Aineq,bineq,Aeq,beq,lb1,ub1,[],options);
if exitflag>=-1
    xc1=exp(ln_x);
    fprintf('Optimization objective=%f\n',obj);
elseif exitflag==-2
    xc1=exp(output.bestfeasible.x);
