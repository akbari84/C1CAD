function [assim_rate,uc,xc,exitflag]=Fun_AssimRateMC_rAcCoA(xc_0,pHc,beta,x_c_H2,x_ub,index)
clc;
...
Aineq=zeros(n_rxn_cf+4,n_met);
bineq=zeros(n_rxn_cf+4,1);
Aeq=zeros(2,n_met);
beq=zeros(2,1);
...
Aineq(n_rxn_cf+4,i_atp)=-1;
Aineq(n_rxn_cf+4,i_adp)=1;
Aineq(n_rxn_cf+4,i_pi)=1;
Aeq(1,i_pyr)=1;
Aeq(2,i_h2)=1;
...
options=optimoptions('fmincon','Algorithm','active-set','EnableFeasibilityMode',true,'SpecifyObjectiveGradient',true,'SubproblemAlgorithm','cg',...
                    'ScaleProblem',true,'StepTolerance',1e-8,'OptimalityTolerance',1e-7,'ConstraintTolerance',1e-7,'MaxIterations',10000,'MaxFunctionEvaluations',60000);
